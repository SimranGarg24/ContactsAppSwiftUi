...
class CoreDataManager{
    ...
    func create(contact: Contact){
        ...
        if !contact.phone.isEmpty {
            var phoneSet = Set<CDPhone>()
            contact.phone.forEach({ phone in
                let cdPhone = CDPhone(context: container.viewContext)
                cdPhone.id = phone.id
                cdPhone.type =  phone.type
                cdPhone.phone = phone.phone
                
                phoneSet.insert(cdPhone)
            })
            cdcontact.toPhone = phoneSet
        }
